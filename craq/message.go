...
type Message struct {
	...
	Version    uint64 // version number for CRAQ
	QueryID    uint64 // query ID for version queries
}
...
func (m *Message) Encode() []byte {
	...
	binary.Write(buf, binary.BigEndian, m.QueryID)
...
func DecodeMessage(data []byte) (*Message, error) {
	...
	binary.Read(buf, binary.BigEndian, &m.QueryID)
